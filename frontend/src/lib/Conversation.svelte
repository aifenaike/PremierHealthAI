<script lang="ts">
  ...
  import { PharmacistSocket, type TranscriptMessage } from "../types";
  ...
  onMount(async () => {
    PharmacistSocket.on("transcript", (x) => {
      console.log(x);
      loading = true;
      live = x.replaceAll("\n", "\n\n");
    });
...
>
  <Tile class="w-full flex justify-between">
    <p class="text-lg">Patient Conversation</p>
    <CircleDash
  ...
  <Content class="overflow-y-scroll">
    ...
    {:else}
      <p class="text-gray-500">Conversation transcripts shows here</p>
    {/if}
  </Content>
</div>
